...
namespace Avalonia.Xaml.Interactions.Animated
{
    public class VerticalScrollViewerAnimatedBehavior : StyledElementBehavior<ScrollViewer>
    {
        ...
        private const double AnimationDuration = 200; // длительность анимации в миллисекундах
        ...
        private void AnimateScroll(double delta)
        {
            ...
            if (_isAnimating)
            {
                // Вычисляем прошедшее время и прогресс текущей анимации
                ...
                // Используем easing для текущего прогресса
                var easing = new QuadraticEaseOut();
                double easedProgress = easing.Ease(progress);

                // Обновляем текущее смещение с учетом easing
                _startOffset = _startOffset + easedProgress * (_targetOffset - _startOffset);

                // Обновляем _targetOffset с новым дельта
                _targetOffset += delta;
        ...
        private async Task Animate()
        {
            while (_isAnimating)
            {
                ...
                await Task.Delay(16);  // Обновление каждый кадр (16 мс)
            }
        }
    }
}